...
%  @@@@hy 2017-3-16
%  修改 YUV422的 输出格式为 YUV422_YVYU 输出数据到 *_422yvyu.yuv
%
%%%%
clc;close all;clear 
YES = 1; NO = 0;
...
%% modify YUYV to YVYU 4:2:2 2017-3-16 BY hy 
% YUV4:4:4 -->> YVYU 4:2:2   
% 四个像素为：[Y0 U0 V0] [Y1 U1 V1] [Y2 U2 V2] [Y3 U3 V3]
%
% 存放的码流：[Y0 V0] [Y1 U1] [Y2 V2] [Y3 U3]
%
% 映射的像素: [Y0 U1 V0] [Y1 U1 V0] [Y2 U3 V2] [Y3 U3 V2]
%%%

len = imgHeight*imgWidth+imgHeight*imgWidth/2+imgHeight*imgWidth/2;
yuv422out = zeros(1,len);
yuv422sampY = Y;
yuv422sampU = U(:,2:2:size(U,2));  % Modify U(:,1:2:size(U,2)) to U(:,2:2:size(U,2))
yuv422sampV = V(:,1:2:size(V,2));  % Modify V(:,2:2:size(V,2)) to V(:,1:2:size(V,2))
yuv422out(1:2:len) = reshape(yuv422sampY',1,[]);  %%% 注意要转置
yuv422out(2:4:len) = reshape(yuv422sampV',1,[]);
yuv422out(4:4:len) = reshape(yuv422sampU',1,[]);
