% %
% % 2016-04-18
% %   1.读取文本中的第一个BLOCK，数据为4:2:2的YUV数据
...
%
%  斌哥：这个422的转换显示脚本先修改图片尺寸Vs*Hs （720P或1080P）,
...
for rcount = 1:Hs/16  %45*16 = 720 如果是1080P的照片请修改45为 1080/16=67
    ...
    for colcount = 1:Vs/16 %80*16=1280 如果是1080P的照片请修改80为 1920/16=120
        ...
        tempuv = fscanf(fid_txtin,'%02x',[20 16])';
        %inblock(1:8,:,posv) = fscanf(fid_txtin,'%2x',[8 20]);

        inblock(1:8,1:10,posu) = tempuv(1:8,1:10); 
        inblock(1:8,11:20,posu) = tempuv(1:8,1:10);
        inblock(9:16,1:10,posu) = tempuv(9:16,1:10);
        inblock(9:16,11:20,posu) = tempuv(9:16,1:10);
        
        inblock(1:8,1:10,posv) = tempuv(1:8,11:20); 
        inblock(1:8,11:20,posv) = tempuv(1:8,11:20);
        inblock(9:16,1:10,posv) = tempuv(9:16,11:20);
        inblock(9:16,11:20,posv) = tempuv(9:16,11:20);
...
%         tempv = imgblock(1:8,1:8,3);
        tempu = imgblock(:,1:8,2);
        tempv = imgblock(:,1:8,3);
        tempy = fliplr(tempy);
        tempu = fliplr(tempu);
        tempv = fliplr(tempv);
        Y = [Y tempy];
    ...
    U = uint8(U)';
    U = reshape(U,1,16*16*Vs/16/2);
    V = uint8(V)';
    V = reshape(V,1,16*16*Vs/16/2);
    YY = [YY Y];
    UU = [UU U];
    VV = [VV V];
end
...
leng = length([YY UU VV]);
yuvstream = zeros(1,leng);
yuvstream(1:2:leng) = YY;
yuvstream(2:4:leng) = UU;
yuvstream(4:4:leng) = VV;
...
% datestr(now) 获取时间函数
%
filename_out = ['yuv422stream_',datestr(now,30),'.yuv'] %
fid_txtout = fopen(filename_out,'wb');
    fwrite(fid_txtout,yuvstream,'uint8');
fclose(fid_txtout);
...
%%
% %输出数据的文件名
% fid_txtout = fopen('my_out_24.yuv','wb');
